...
class AuditLogic extends BaseLogic
{
    ...
    public function wechatAudit(array $param)
    {
        ...
        $city = '宁波';
        if (!empty($city)) {
            $where .= ' and (l.city like "'.$city.'%" or t.city like "'.$city.'%" ) ';
        }
        $field = 'body_name,phone,wechat,u.remark,quick_remark,u.id,l.city as lcity,t.city as tcity';
        $info = $this->user->getWechatAudit($where,$page,$size,$field);
        $count = $this->user->getWechatAuditCount($where,$field);
        if (!empty($info)) {
