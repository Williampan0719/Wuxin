...
class UserLogic extends BaseLogic
{
    ...
    public function searchUserList(array $param)
    {
        ...
        //助教账号
        $adminModel = new RoleAdmin();
        $adminDetail = $adminModel->getOne(['admin_id'=>$param['admin_id']],'role_id');
        $admin = new Admin();
        $city = $admin->getFieldByKey(['admin_id'=>$param['admin_id']],'city');


        if ($adminDetail['role_id'] == 16) {
            $ass = new Assistant();
            $ass_id = $ass->getValue(['admin_id'=>$param['admin_id']],'id');
            $where .= ' and u.assistant = '.$ass_id;
        }
        ...
        if (!empty($param['city']) || !empty($city)) { //地区搜索或加盟商
            if (!empty($city)) {
                $param['city'] = $city;
            }
        ...
        if (isset($param['certime']) && is_numeric($param['certime'])) {
            if ($param['certime'] == 2) {
                ...
                $where .= ' and ((role = 1 and certime = 0 and (body_name = "" or wechat = "")) or (role = 2 and certime = 0 and (body_name = "" or wechat = "" or f.id is null))) ';
            }
        }
        $field = 'f.id as fid,c.type as ctype,u.id,u.role,u.body_name,u.phone,u.wechat,u.certime,l.city as lcity,t.city as tcity,l.is_order as lorder,t.is_order as torder,u.assistant,u.resource,u.remark,u.quick_remark,u.nickname,u.level';
        $user = new User();
     ...
     * @DateTime 2018-05-21
     *
     * @description 后台用户可见的前台列表
     * @param array $param
     * @return array
     */
    public function frontList(array $param)
    {
        $page = $param['page'] ?? 1;
        $size = $param['size'] ?? 10;
        $user = new User();
        $tutor = new Tutor();
        $learn = new Learn();
        $role = $user->getValue($param['front_uid'],'role');
        $field = 'u.*';
        if ($role == 1) { //看家教
            $userInfo = $learn->getOne($param['front_uid']);
            $where = 'teach_range != "" and teach_subject != "" and is_order = 1 and u.certime > 1 and t.city = "'.$userInfo['city'].'"';
            $where2 = $where;
            if ($userInfo['learn_range'] != '') {
                $where .= ' and (teach_range like "'.$userInfo['learn_range'].',%" or teach_range like "%,'.$userInfo['learn_range'].',%")'; //考虑10，11
            }
            if ($userInfo['learn_subject'] != '') {
                $where .= ' and (teach_subject like "'.$userInfo['learn_subject'].',%" or teach_subject like "%,'.$userInfo['learn_subject'].',%")';
            }
            $field = 'u.id,u.body_name,u.phone,u.nickname,u.role,u.resource,u.level,u.remark,u.assistant,u.certime,u.quick_remark,t.city,t.id as tid,uid,is_order,lng,lat,geo_hash';
        }elseif ($role == 2) { //看家长
            $userInfo = $tutor->getOne($param['front_uid']);
            ...
            if ($userInfo['teach_range'] != '') {
                ...
                $where .= ' and concat(",",learn_range,",") in (99,'.$userInfo['teach_range'].'99)';
            }
            if ($userInfo['teach_subject'] != '') {
                //$userInfo['teach_subject'] = substr($userInfo['teach_subject'],0,strlen($userInfo['teach_subject'])-1);
                $where .= ' and concat(",",learn_subject,",") in (99,'.$userInfo['teach_subject'].'99)';
            }
            $field = 'u.id,u.body_name,u.phone,u.nickname,u.role,u.resource,u.level,u.remark,u.assistant,u.certime,u.quick_remark,l.city,l.id as lid,uid,is_order,lng,lat,geo_hash';
        }else{
        ...
        //同步列表搜索条件
        if (!empty($param['role'])) {
            $where .= ' and u.role = '.$param['role'];
        }
        //助教账号
        $admin_role = new RoleAdmin();
        $role_id = $admin_role->getValue(['admin_id'=>$param['admin_id']],'role_id');
        if ($role_id == 16) {
            $ass = new Assistant();
            $ass_id = $ass->getValue(['admin_id'=>$param['admin_id']],'id');
            $where .= ' and u.assistant = '.$ass_id;
        ...
        if (!empty($param['city'])) {
            if ($role == 1) {
                $where .= ' and t.city like "'.$param['city'].'%"';
            }elseif ($role == 2) {
                $where .= ' and l.city like "'.$param['city'].'%"';
            }
        }
        if (isset($param['is_order']) && is_numeric($param['is_order'])) {
            if ($role == 1) {
                $where .= ' and t.is_order = '.$param['is_order'];
            }elseif ($role == 2) {
                $where .= ' and l.is_order = '.$param['is_order'];
            }
        }
        ...
        if (isset($param['is_match']) && $param['is_match'] == 0) { //除匹配部分外
            if ($role == 1) {
                $total1 = $user->tutorMatchCount($where);
                $total_all = $user->tutorMatchCount($where2);
                $total2 = $total_all - $total1;
                $total = $total2;
                //排除匹配到的部分
                $ids = $user->tutorMatchIds($where,'u.id');
                if (!empty($ids)) {
                    $ids = implode(',',array_column($ids,'id'));
                    $where2 .= ' and uid not in ('.$ids.')';
                }
                $list = $user->tutorMatchPageSelect($where2,$field,$page,$size,'distance asc');
            }else{
                $total1 = $user->learnMatchCount($where);
                $total_all = $user->learnMatchCount($where2);
                $total2 = $total_all - $total1;
                $total = $total2;
                //排除匹配到的部分
                $ids = $user->learnMatchIds($where,'u.id');
                if (!empty($ids)) {
                    $ids = implode(',',array_column($ids,'id'));
                    $where2 .= ' and uid not in ('.$ids.')';
                }
                $list = $user->learnMatchPageSelect($where2,$field,$page,$size,'distance asc');
            }
        }else{
            if ($role == 1) {
                $total1 = $user->tutorMatchCount($where); //匹配总数
                $total_all = $user->tutorMatchCount($where2); //总数
                $total2 = $total_all - $total1;
                $total = $total1;
                $list = $user->tutorMatchPageSelect($where,$field,$page,$size,'distance asc');
            }else{
                $total1 = $user->learnMatchCount($where);
                $total_all = $user->learnMatchCount($where2);
                $total2 = $total_all - $total1;
                $total = $total1;
                $list = $user->learnMatchPageSelect($where,$field,$page,$size,'distance asc');
            }
        }
        ...
        return $this->ajaxSuccess(104,['list'=>$list,'total'=>$total,'total1'=>$total1,'total2'=>$total2,'total_all'=>$total_all,'role'=>$role]);
    }
    ...
    public function tutorPanel(array $param)
    {
        ...
        $ass = new Assistant();
        $re = new Resource();
        $userInfo = $user->finds($param['uid'],'id,role,portrait,openid,wechat,qrcode,phone,sex,body_name,addtime,wechat_status,idcard,nickname,assistant,resource,remark,logintime,device,level,quick_remark');
        ...
        if (!empty($userCer)) {
            ...
            $userInfo['education_time'] = $userInfo['education'] == '认证通过' ? $auth[2]['audittime'] : '';
        ...
        $userCerInfo = $cerInfo->getOne(['uid'=>$param['uid']],'school,diploma,professional,addschool');
        ...
        if (!empty($userCerInfo)) {
            ...
            //判断毕业状态
            if ($userCerInfo['addschool']) {
                $userCerInfo['addschool'] = str_replace(' ','',$userCerInfo['addschool']);
                $addtime = date('Y')-substr($userCerInfo['addschool'],0,4);
                if ($userCerInfo['diploma'] == '本科') {
                    $userInfo['edu_now'] = $addtime > 4 ? '毕业'.($addtime-4).'年' : '大'.$temp[$addtime];
                }elseif ($userCerInfo['diploma'] == '专科') {
                    $userInfo['edu_now'] = $addtime > 3 ? '毕业'.($addtime-3).'年' : '大'.$temp[$addtime];
                }
            }
        }
    ...
    public function learnPanel(array $param)
    {
        ...
        $ass = new Assistant();
        $re = new Resource();
        $userInfo = $user->finds($param['uid'],'id,role,portrait,openid,wechat,sex,qrcode,phone,body_name,addtime,wechat_status,idcard,nickname,assistant,resource,remark,logintime,device,level,quick_remark');
        if (!empty($userInfo['assistant'])) {
            $userInfo['assistant'] = $ass->getValue(['id'=>$userInfo['assistant']],'name');
        }else{
            $userInfo['assistant'] = '';
    ...
    public function getComplainList(array $param)
    {
        ...
        if (!empty($list)) {
            ...
            foreach ($list as $key => $value) {
                ...
                $be = $user->finds($value['to_uid'],'wechat,phone,portrait,nickname');
