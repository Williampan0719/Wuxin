...
class User extends BaseAdmin
{
    ...
    /**
     * @api {get} /backend/user/front-list 匹配列表
     * @apiGroup user
     * @apiName  front-list
     * @apiVersion 1.0.0
     * @apiParam {int} page 当前页
     * @apiParam {int} size 每页数
     * @apiParam {int} is_match 是否匹配
     * @apiParam {int} front_uid 用户id
     * @apiSuccess {int} status 调用状态 1-调用成功 0-调用失败
     * @apiSuccess {int} code   仅供参考
     * @apiSuccess {string} message 提示消息
     * @apiSuccess {Object} data 数据部分,忽略
     * @apiSampleRequest http://apitest.jkxxkj.com/backend/user/front-list
     * @apiSuccessExample {json} Response 200 Example
     * {
     *  "status": 1,
     *  "message": "获取成功",
    ...
    public function frontList()
    {
        $params = $this->param;
        $params['admin_id'] = self::$admin_id;
        $result = $this->user->frontList($params);
        return $result;
    }
